...
```
## Future Improvements
- **Algorithmic Optimizations**: Implement ```text $O(N \log N)$ ``` sorting-based and ```text $O(N)$ ``` Hash-based duplicate detection algorithms to compare with quadratic runtime.
